...
function StripeBookingFields(props: PaymentBookingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitError('');
    if (!stripe || !elements || !props.clientName.trim() || !props.clientEmail.trim() || !props.clientPhone.trim()) return;
...
function BookingFields({
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-7 xl:grid-cols-[minmax(340px,0.9fr)_minmax(540px,1.1fr)]">
      ...
      <InputBlock label="Telefono WhatsApp" icon={<Phone className="w-5 h-5" />}>
        <input id="input-client-phone" type="tel" required value={clientPhone} onChange={(e) => onClientPhoneChange(e.target.value)} placeholder="+34 612 345 678" className="booking-input" />
      </InputBlock>
